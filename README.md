...
如果你用 TweetClaw 或 Xquik 先筛选公开推文、活动结果或内容灵感,可以把人工确认后的标题和正文整理成 `slides.json` 再交给本 skill。建议只带 `title`、`body`、`slug`、`sourceUrl` 这类公开上下文;不要把账号 Cookie、令牌或草稿凭据写进 slides 文件。生成器仍然只负责本地排版和出图。
